...
| `npm run probe:detection` | **The test suites have stopped detecting.** It applies thirty-seven known defects, asserts each one is caught, and restores. A refactor that guts a suite still reports green everywhere else — this is what notices |
